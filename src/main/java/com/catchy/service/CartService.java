...
public class CartService {
    ...
    public BigDecimal getCartTotalBySession(String sessionId) {
        List<CartItem> cartItems = getCartItemsBySession(sessionId);
        return cartItems.stream()
                .map(item -> item.getProduct().getPrice().multiply(BigDecimal.valueOf(item.getQuantity())))
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    }
    ...
    @Transactional
    public CartItem addToCartBySession(String sessionId, Long productId, Integer quantity) {
        Product product = productRepository.findById(productId)
                .orElseThrow(() -> new RuntimeException("Product not found"));
        ...
        Optional<CartItem> existingCartItem = cartItemRepository.findBySessionIdAndProductId(sessionId, productId);
    ...
    @Transactional
    public CartItem updateCartItemQuantityBySession(String sessionId, Long cartItemId, Integer quantity) {
        CartItem cartItem = cartItemRepository.findById(cartItemId)
                .orElseThrow(() -> new RuntimeException("Cart item not found"));

        if (cartItem.getSessionId() == null || !sessionId.equals(cartItem.getSessionId())) {
            throw new RuntimeException("Unauthorized");
        }
    ...
    @Transactional
    public void removeFromCartBySession(String sessionId, Long cartItemId) {
        CartItem cartItem = cartItemRepository.findById(cartItemId)
                .orElseThrow(() -> new RuntimeException("Cart item not found"));
